"""
Sourced from here: https://github.com/jmcarp/sqlalchemy-postgres-copy
Extracted into the repository to avoid the psycopg2 dependency
...
def copy_from(source, dest, engine_or_conn, columns=(), **flags):
    ...
    copy = "COPY {} {} FROM STDIN {}".format(
        relation, formatted_columns, formatted_flags,
    )
...
def format_flag(value):
    return six.text_type(value).upper() if isinstance(value, bool) else repr(value)
